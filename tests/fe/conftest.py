...
# ─── Common Login Helper (Session-scoped) ────────────────────────────────────────

@pytest.fixture(scope="session")
def logged_in_page_factory(browser: Browser):
    """
    Factory fixture: คืน page ที่ login เป็น role ที่กำหนดแล้ว
    scope="session" = login ครั้งเดียวต่อ session (ประหยัดเวลา)

    ใช้ใน tests ที่ต้องการ pre-authenticated page:
        def test_something(logged_in_page_factory):
            page = logged_in_page_factory("admin")
            page.goto("/customers")
    """
    from helpers.common_web import login_as, TEST_USERS

    # เก็บ page สำหรับแต่ละ role (cache)
    _pages: dict = {}

    def _get_page(role: str) -> Page:
        # ถ้า login แล้วและ page ยังเปิดอยู่ → reuse
        if role in _pages and not _pages[role].is_closed():
            return _pages[role]

        # สร้าง context ใหม่ สะอาด สำหรับ role นี้
        ...
        # Login เป็น role ที่กำหนด
        login_as(page, role=role)
        _pages[role] = page
        return page

    yield _get_page

    # Teardown: ปิดทุก page เมื่อ session เสร็จสิ้น
